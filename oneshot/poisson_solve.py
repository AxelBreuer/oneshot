...
def poisson_solve(c0, c2, f, solver_type='sparse'):

    """
    Return u, solution of the second order elliptic PDE

        - c2(x,y) Laplacian u(x,y) + c0(x,y) u(x,y) = f(x,y)
    ...
    for jj in range(1, Ny-1):
        ...
        upper2[mm(1, jj): mm(Nx-1, jj)] = -c2[1:Nx-1, jj]
        upper[mm(1, jj): mm(Nx-1, jj)] = -c2[1:Nx-1, jj]
        main[mm(1, jj):mm(Nx-1, jj)] = 4.0 * c2[1:Nx-1, jj] + c0[1:Nx-1, jj]
        lower[mm(1, jj)-lower_offset: mm(Nx-1, jj)-lower_offset] = -c2[1:Nx-1, jj]
        lower2[mm(1, jj)-lower2_offset: mm(Nx-1, jj)-lower2_offset] = -c2[1:Nx-1, jj]
